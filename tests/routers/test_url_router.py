...
@pytest.mark.anyio
@pytest.mark.unit
async def test_create_short_url_success(mock_client, mock_url_service, mock_url_obj):
  mock_url_service.get_or_create.return_value = mock_url_obj

  result = await mock_client.post("/shorten", json={"long_url": mock_url_obj.long_url})
  data = result.json()
...
@pytest.mark.anyio
@pytest.mark.unit
async def test_create_short_url_failure(mock_client, mock_url_service, mock_url_obj):
...
@pytest.mark.anyio
@pytest.mark.unit
async def test_create_short_url_wrong_input_failure(mock_client):
  result = await mock_client.post("/shorten", json={"long_url": "not valid url"})
  data = result.json()
...
@pytest.mark.anyio
@pytest.mark.unit
async def test_fetch_long_url_success(mock_client, mock_url_service, mock_url_obj):
  mock_url_service.fetch_long_url.return_value = mock_url_obj

  result = await mock_client.get(f"/{mock_url_obj.short_code}", follow_redirects=False)
...
@pytest.mark.anyio
@pytest.mark.unit
async def test_fetch_long_url_not_found(mock_client, mock_url_obj, mock_url_service):
...
@pytest.mark.anyio
@pytest.mark.unit
async def test_fetch_stats_success(mock_client, mock_url_service, mock_url_obj, mock_short_url):
  mock_url_service.fetch_stats.return_value = mock_url_obj

  result = await mock_client.get(f"/stats/{mock_short_url}")
  data = result.json()
...
@pytest.mark.anyio
@pytest.mark.unit
async def test_fetch_stats_short_url_not_found(mock_client, mock_url_service, mock_short_url):
...
async def test_url_lifecycle_happy_path(integration_client, payload_long_url):
  create_short_url_result = await integration_client.post("/shorten", json=payload_long_url)
  ...
  fetch_long_url_result = await integration_client.get(f"/{short_code}", follow_redirects=False)

  assert fetch_long_url_result.status_code == 307
  assert fetch_long_url_result.headers["location"] == payload_long_url["long_url"]

  fetch_stats_result = await integration_client.get(f"/stats/{short_url}")
